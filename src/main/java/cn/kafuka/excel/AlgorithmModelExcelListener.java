...
@Setter
@Component
public class AlgorithmModelExcelListener extends AnalysisEventListener<AlgorithmModelExcelVo> {
    ...
    private List<AlgorithmModelExcelVo> algorithmModelExcelVoList = new ArrayList<>();
    ...
    @Tolerate
    public AlgorithmModelExcelListener() {}

    public AlgorithmModelExcelListener(AlgorithmModelService algorithmModelService, Validator validator, FieldDupValidator fieldDupValidator) {
        this.algorithmModelService = algorithmModelService;
    ...
    @Override
    public void invoke(AlgorithmModelExcelVo algorithmModelExcelVo, AnalysisContext analysisContext) {

        //1.校验基本参数
        Integer currentRowNum = analysisContext.readRowHolder().getRowIndex();
        validate(algorithmModelExcelVo,currentRowNum);

        //2.请求参数处理

        //3.当前行数据添加到列表
        algorithmModelExcelVoList.add(algorithmModelExcelVo);
    }

    //2.参数校验
    private void validate(AlgorithmModelExcelVo r, Integer rowNum) {
        //(1).自定义注解的参数校验(重复字段校验)
    ...
    private boolean customValidate(AlgorithmModelExcelVo r, Integer rowNum) {
    ...
    //--2.hibernate注解 参数校验
    private void hibernateValidate(AlgorithmModelExcelVo r, Integer rowNum) {
        Set<ConstraintViolation<AlgorithmModelExcelVo>> validateSet = validator.validate(r, Default.class);
        if (validateSet != null && !validateSet.isEmpty()) {
            ConstraintViolation<AlgorithmModelExcelVo> constraint = validateSet.stream().findAny().orElse(null);
    ...
    @Transactional
    void importDataBase() {
        algorithmModelExcelVoList.forEach(e -> {
            AlgorithmModelReqDto algorithmModelReqDto = VoPoConverterUtil.copyProperties(e, AlgorithmModelReqDto.class);
            algorithmModelService.insertOrUpdateAlgorithmModel(algorithmModelReqDto);
        });
    }
}