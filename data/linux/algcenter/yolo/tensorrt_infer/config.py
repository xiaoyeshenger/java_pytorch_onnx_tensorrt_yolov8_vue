...
# 烟雾
CLASSES_DET_SMOKE = ('烟雾','火')
COLORS_SMOKE = {
    cls: [random.randint(0, 255) for _ in range(3)]
    for i, cls in enumerate(CLASSES_DET_SMOKE)
}


# 烟火
CLASSES_DET_FIRE = ('火','烟雾')
COLORS_FIRE = {
    cls: [random.randint(0, 255) for _ in range(3)]
    for i, cls in enumerate(CLASSES_DET_FIRE)
}


# 安全帽
CLASSES_DET_SAFETY_HAT = ('安全帽', '口罩', 'NO-Hardhat', '未戴口罩', '未戴安全帽' 'Vest', 'Person', 'Safety Cone', 'Safety Vest', 'machinery', 'vehicle')
# colors for per classes
COLORS_SAFETY_HAT = {
    cls: [random.randint(0, 255) for _ in range(3)]
    for i, cls in enumerate(CLASSES_DET_SAFETY_HAT)
}

# 城市裸土
CLASSES_DET_BARE_SOIL = ('裸土','其他')
COLORS_BARE_SOIL = {
    cls: [random.randint(0, 255) for _ in range(3)]
    for i, cls in enumerate(CLASSES_DET_BARE_SOIL)
}

# 城市裸土
CLASSES_DET_RIVER_REFUSE = ('河道垃圾','其他')
COLORS_RIVER_REFUSE = {
    cls: [random.randint(0, 255) for _ in range(3)]
    for i, cls in enumerate(CLASSES_DET_RIVER_REFUSE)
}

# 电瓶车头盔
CLASSES_DET_EBIKE = ("未戴头盔","骑行者","头盔")
COLORS_EBIKE = {
    cls: [random.randint(0, 255) for _ in range(3)]
    for i, cls in enumerate(CLASSES_DET_EBIKE)
}

# 道路墙体裂缝
CLASSES_DET_CRACK = ('裂缝1','裂缝2','裂缝3','裂缝4','裂缝5','裂缝6')
COLORS_CRACK = {
    cls: [random.randint(0, 255) for _ in range(3)]
    for i, cls in enumerate(CLASSES_DET_CRACK)
}

# 车牌识别
CLASSES_DET_CAR_PLATE = ('车牌1','车牌2','车牌3','车牌4','车牌5')
COLORS_CAR_PLATE = {
    cls: [random.randint(0, 255) for _ in range(3)]
    for i, cls in enumerate(CLASSES_DET_CAR_PLATE)
}


# 罂粟-植物识别
CLASSES_DET_PLANT = ('罂粟','其他植物')
COLORS_PLANT = {
    cls: [random.randint(0, 255) for _ in range(3)]
    for i, cls in enumerate(CLASSES_DET_PLANT)
}
